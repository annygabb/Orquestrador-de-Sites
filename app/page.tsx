  ...
  return <>
    ...
    <main className="sales-page">
      ...
      <section className="pricing-section sales-pricing" id="plano" aria-labelledby="pricing-title">
        ...
        <article className="pricing-card" data-reveal><div className="price-first"><span>Ativação + 30 dias</span><strong>{moneyFromCents(ACTIVATION_PRICE_CENTS)}</strong><small>pagamento inicial</small></div><div className="price-renewal"><span>Depois</span><strong>{moneyFromCents(RENEWAL_PRICE_CENTS)}<em>/mês</em></strong><small>cancelável quando quiser</small></div><ul><li>Biblioteca de skills e referências em evolução</li><li>Adicione novas skills sempre que seu processo mudar</li><li>Combine referências com itens personalizados para cada projeto</li><li>Reaproveite decisões aprovadas e entregue mais contexto para a IA</li></ul><MovingBorderButton as="a" href={user ? "/pagamento" : "/entrar?intent=signup"} containerClassName="moving-cta moving-cta--wide" className="moving-cta__inside moving-cta__inside--primary">{user ? "Ativar acesso" : "Criar minha conta"}<span aria-hidden="true">↗</span></MovingBorderButton></article>
      </section>
